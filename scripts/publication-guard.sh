...
check_path() {
  ...
  base=${lower##*/}
  case "$lower" in
    agents/*|*/agents/*|brain/*|credentials/*|*/credentials/*|factory/*|internal/*|keys/*|*/keys/*|private/*|*/private/*|prompts/*|policies/*|rules/*|secrets/*|*/secrets/*|skills/*|logs/*|*/logs/*|validation/*|.startwave-agent/*|tools/*|games/mir_kameadril/*|assets/data/bdo-*) block "$file" 'private, Factory, or owner-review path'; return 1 ;;
...
check_content() {
  ...
  if grep -E -q -- 'sb_secret_[A-Za-z0-9_-]{20,}' "$content_file"; then block "$file" 'contains a Supabase secret key'; fi
  credential_file=$content_file
  publishable_scan_file=
  if [ "$file" = "GAMES/bdo/site/assets/js/bdo-live-data.js" ]; then
    publishable_assignments=$(grep -E -c -- "^  const SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_[A-Za-z0-9_-]+';$" "$content_file" || :)
    publishable_usages=$(grep -E -i -c -- 'apikey[[:space:]]*:[[:space:]]*SUPABASE_PUBLISHABLE_KEY' "$content_file" || :)
    if [ "$publishable_assignments" -ne 1 ] || [ "$publishable_usages" -ne 1 ]; then
      block "$file" 'invalid Supabase publishable key contract'
    else
      publishable_scan_file=$(mktemp "${TMPDIR:-/tmp}/startwave-guard-publishable.XXXXXX")
      sed -E 's/apikey[[:space:]]*:[[:space:]]*SUPABASE_PUBLISHABLE_KEY/PUBLIC_BROWSER_KEY/g' "$content_file" > "$publishable_scan_file"
      credential_file=$publishable_scan_file
    fi
  fi
  if grep -E -i -q -- '(api[_-]?key|access[_-]?token|client[_-]?secret|password)[[:space:]]*[:=]' "$credential_file"; then
    block "$file" 'contains a credential assignment'
  fi
  if grep -E -q -- '/Users/[A-Za-z0-9._-]+/|[A-Za-z]:\\Users\\[^\\]+' "$content_file"; then block "$file" 'contains an absolute local user path'; fi
  [ -z "$publishable_scan_file" ] || rm -f "$publishable_scan_file"
  rm -f "$content_file"
}
